...
import { RequiredFileInterceptor } from './required-file.interceptor';

describe(`UT:${RequiredFileInterceptor.name}`, () => {
  const enum should {
  ...
  beforeEach(() => {
    factory = RequiredFileInterceptor('file');
  ...
  it(should.continuetoNextHandle, async () => {
    ...
    const mockContext = {
      switchToHttp: () => ({
        getRequest: () => ({ file: { originalname: 'test.png' } }),
      }),
    } as unknown as ExecutionContext;
    const mockNext: CallHandler = {
      handle: jest.fn().mockReturnValue(of(null)),
